...
p1 <- ggplot(experiment,
  ...
  # geom_text(y=1.2,label = "***",size=3) +
  labs(x = '',y=expression( Mean(AS) - Mean(RS))) + scale_y_continuous(breaks=c(1000,0,-1000),limits=c(-1000,1000))
...
p2 <- ggplot(experiment,
  ...
  # scale_colour_manual(values = c('Orange','Purple')) +
  # geom_text(y=1.2,label = "***",size=3) +
  labs(x = '',y=expression(F[max](AS) - F[max](RS))) + scale_y_continuous(breaks=c(1000,0,-1000),limits=c(-1000,1000))

ggsave('../Plots/FigS4.png',ggarrange(p1,p2,common.legend = TRUE,labels='AUTO'),height=5,width=10)
