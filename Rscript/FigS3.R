...
mapping_file = mapping_file[cost_mean==0.1 & selected_function == 'f1_additive',]
...
p2 <- ggplot(df_b,
  ...
  scale_colour_manual(values = c('Orange','Purple')) + 
  # geom_text(y=1500,label = "***",size=3) +
  labs(x = '',y=expression(Q==F[max](AS) - F[max](NS) ))+ scale_y_continuous(breaks=c(2000,0,-2000),limits=c(-2000,2000))
...
ggsave('../Plots/FigS3.png',grid.arrange(ggarrange(p1,p2,labels=c('A','B'),common.legend=TRUE), 
                                         ggarrange(p4,p5,p6,p7,p8,p9,labels=c('C','D','E','F','G','H')),nrow=2,ncol=1,heights=c(3,3)),height=8,width=8)