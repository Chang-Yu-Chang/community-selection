...
t_Synthetic = t[grep('iteration_3-3-synthetic_community-migration',t$exp_id)]
t_Synthetic$Community = 'Synthetic'
t_Synthetic = t_Synthetic[Well != t_Synthetic[Transfer==20 & CommunityPhenotype == Maximum]$Well]
t_sel = t[grep('iteration_3-3-selected_community-migration',t$exp_id)]
t_sel$Community = 'DE'
...
p1_inset = readPNG( "../Plots/Cartoons/Fig4B.png", TRUE)
p1_inset <- rasterGrob(p1_inset, interpolate=TRUE)
...
p2 <- ggplot() + 
  ...
  theme_pubr() + labs(x= 'Generation' ,y = 'F',col='') + scale_y_continuous(breaks = c(-500,2000),limits=c(-500,2500)) + 
  scale_x_continuous(breaks = c(0,20,40),limits=c(0,46))+ #guides(col=FALSE) +
  annotation_custom(p1_inset,xmin=0,xmax=40,ymin=1700,ymax=2600)+ 
  annotate('text',x=13,y=-100,label=expression(R==1 - frac(2* "|" * F[max] - F^'*' * "|",F[max] + "|" * F[max] - F^'*' * "|")),size=4)  +
  annotate('text', x = 45,y = 1646.7, label = expression(F[max]),size=4) +
  annotate('text', x = 45,y=103.5,label=expression(F^'*'),size=4) +
  annotate('segment',x=40.5,xend=43.5,y=103.5,yend=103.5,linetype = 2,col = '#7570B3') +
  annotate('segment',x=20.5,xend=42,y=1646.7,yend=1646.7,linetype=2,col = '#7570B3') + guides(col=FALSE) +
...
p3 <- ggboxplot(t_final,x='Community',y='CommunityPhenotype',col='Community',palette = "dark2",
                add = "jitter",legend='right',shape=1,outlier.size=1,outlier.colour='white',dot.size=0.1) + 
  ...
  scale_y_continuous(breaks=c(-500,1500),limits=c(-500,1850)) + 
...
p4 <- ggboxplot(t_final,x='Community',y='RS',col='Community',palette = "dark2",
                add = "jitter",legend='right',shape=1,outlier.size=1,outlier.colour='white',height=0) + 
  guides(col=FALSE) +   
  stat_compare_means(paired=TRUE,comparisons = comparisons,method='t.test',label.y=c(1.1,1.25,1.4),
                     symnum.args = list(cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 1), symbols = c("****", "***", "**", "*", "ns"))) +
  labs(x='',y='R') +
  scale_colour_manual(values = c('#1B9E77','#D95F02','#7570B3')) + guides(fill=FALSE)+
  scale_y_continuous(breaks=c(0,1),limits=c(-0.16,1.45)) + 
  theme(axis.title=element_text(size=12),axis.text.x=element_text(angle=-45,colour=c('#1B9E77','#D95F02','#7570B3')),
        axis.title.y = element_text(margin = margin(t = 0, r = 0, b = 0, l = 0))) 


top = ggarrange(p1,p2,width=c(2,1))


t_Synthetic_all = mapping_file[grep('iteration_3.*synthetic_community-migration',mapping_file$exp_id)]
t_Synthetic_all$Community = 'Synthetic'
t_sel_all = mapping_file[grep('iteration_3.*selected_community-migration',mapping_file$exp_id)]
