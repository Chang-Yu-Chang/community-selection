...
mapping_file$file2 = paste('../data/raw/deprecated/',mapping_file$exp_id,'_function.txt',sep='')

t1 = mapping_file[grep('.*synthetic_community-migration',mapping_file$exp_id),]
t1 = t1 [is.na(n_migration_ds),]
t_a = merge(rbindlist(lapply(t1$file,fread)),t1)
t_b = merge(rbindlist(lapply(t1$file2,fread)),t1)
...
ggsave('../Plots/Fig4_old.png',ggarrange(top,NULL,bottom,nrow=3,ncol=1,heights=c(4,0.1,3)),height=7/1,width=10.5/1.1)