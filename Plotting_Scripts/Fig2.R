...
fake_A = A[Transfer ==21] #This connects the old wells with the new wells.
...
B[,Mean_F_Mean := mean(F_Mean),by = list(Popsize_mean)]
B[,SD_F_Mean := sd(F_Mean),by = list(Popsize_mean)]
...
p1 <- rasterGrob(readPNG( "../Plots/Cartoons/Fig2A.png", TRUE), interpolate=TRUE)
p2 <- ggplot() + 
...
p3_inset <- ggplot() + 
  geom_line(B[Well == 'W0'],mapping = aes(x=Popsize_mean,y=Mean_F_Mean),col='Purple') + 
  geom_ribbon(B[Well == 'W0'],mapping = aes(x=Popsize_mean,ymax=Mean_F_Mean +SD_F_Mean,ymin = Mean_F_Mean -SD_F_Mean),fill='Purple',alpha=0.2) + 
  geom_line(B[Well == 'W0'],mapping = aes(x=Popsize_mean,y=Mean_F_Max),col='Orange') + 
  geom_ribbon(B[Well == 'W0'],mapping = aes(x=Popsize_mean,ymax=Mean_F_Max + SD_F_Max,ymin = Mean_F_Max - SD_F_Max),fill='Orange',alpha=0.2) + 
  xscale("log2",.format=FALSE) +
  theme_pubr()  + labs(x='',y='') +
...
p9 <- ggplot(H,aes(x= Pmax,y=Omax)) +
  ...
  theme(axis.text = element_text(size=8),axis.title = element_text(size=8))
top = ggarrange(p1,ggarrange(p2,p3,labels=c('B','C'),nrow=2,ncol=1),labels=c('A'),widths=c(2,1))
bottom = ggarrange(p4,p5,p6,p7,p8,p9,nrow=2,ncol=3,labels=c('D','E','F','G','H','I'))
ggsave('../Plots/Fig2.png',ggarrange(top,bottom,ncol=1,nrow=2,heights=c(6,6)),width=8,height=8)