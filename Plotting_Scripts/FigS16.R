...
mapping_file = mapping_file[l ==0 & metacommunity_sampling == 'Power' & 
                              response == 'type III' & 
                              (selected_function == 'f1_additive' | selected_function=='f1a_additive') &
                              cost_distribution=='Norm',]
mapping_file$phi_type = '~norm(0,1)'
mapping_file[phi_mean==1,]$phi_type = '~norm(1,1)'
mapping_file[phi_distribution=='Uniform',]$phi_type = '~unif(0,1)'
mapping_file[selected_function == 'f1a_additive',]$phi_type = '~norm(0,1)ber(0.2)' 
...
mapping_file2 = mapping_file2[l ==0 & metacommunity_sampling == 'Power' & 
                                ...
                                (selected_function == 'f1_additive' | selected_function=='f1a_additive') &
                                cost_distribution=='Norm',]
mapping_file2$phi_type = '~norm(0,1)'
mapping_file2[phi_mean==1,]$phi_type = '~norm(1,1)'
mapping_file2[phi_distribution=='Uniform',]$phi_type = '~unif(0,1)'
mapping_file2[selected_function == 'f1a_additive',]$phi_type = '~norm(0,1)ber(0.2)' 
...
mapping_file3 = mapping_file3[l ==0 & metacommunity_sampling == 'Power' & 
                                ...
                                (selected_function == 'f1_additive' | selected_function=='f1a_additive') &
                                cost_distribution=='Norm',]
mapping_file3$phi_type = '~norm(0,1)'
mapping_file3[phi_mean==1,]$phi_type = '~norm(1,1)'
mapping_file3[phi_distribution=='Uniform',]$phi_type = '~unif(0,1)'
mapping_file3[selected_function == 'f1a_additive',]$phi_type = '~norm(0,1)ber(0.2)' 
...
# df2 = df2[!(DE_Strategy == 'Resource Shift' & phi_type == 'Target Resource')]  #Rshift strategy not applicable to this function
...
df3[,rank :=frank(-CommunityPhenotype),by=list(seed,Transfer,bottleneck_size,phi_type)]
df3 = df3[bottleneck_size %!in% c(1e-4,1e-5)]
df3 = df3[Transfer==40]
df3$Popsize_mean = 1000*df3$scale*df3$bottleneck_size*df3$dilution
df3[,F_Mean := mean(CommunityPhenotype),by = list(seed,Transfer,bottleneck_size,phi_type)]
df3[,F_Max := max(CommunityPhenotype),by = list(seed,Transfer,bottleneck_size,phi_type)]
df3[,Mean_F_Mean := mean(F_Mean),by = list(Popsize_mean,phi_type)]
df3[,SD_F_Mean := sd(F_Mean),by = list(Popsize_mean,phi_type)]
df3[,Mean_F_Max := mean(F_Max),by = list(Popsize_mean,phi_type)]
df3[,SD_F_Max := sd(F_Max),by = list(Popsize_mean,phi_type)]
df3 = df3[Well == 'W0']
...
df4_control[,Control_Maximum:=max(CommunityPhenotype),by=list(exp_id,Transfer,phi_type)]
df4_control = df4_control[Well == 'W0' & Transfer ==40]
df4_control = df4_control[order(df4_control$exp_id),]
df4_control = df4_control[,c('seed','phi_type','Control_Maximum')]
...
df4 = merge(df4,df4_control,by=c('seed','phi_type'))
...
df5[,Fmax :=max(CommunityPhenotype),by=list(exp_id,Transfer,phi_type)]
df5[,R :=mean(RS),by=list(exp_id,Transfer,phi_type)]
df5[,Fmean :=mean(CommunityPhenotype),by=list(exp_id,Transfer,phi_type)]
df5$Fmax2 = t20_2$Fmax
df5 = df5[Well=='W0']
my_comparisons2 <- list( c("NS", "DE"), c("DE", "Synthetic"), c("NS", "Synthetic") )

df1$phi_type= factor(df1$phi_type,levels=c( "~norm(0,1)","~norm(1,1)","~unif(0,1)","~norm(0,1)ber(0.2)"),labels=c(expression(paste(Phi[i],'~Normal(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(1,1)')),
                                                                                             expression(paste(Phi[i],'~Uniform(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(0,1) x Bernoulli(0.2)'))))
df2$phi_type= factor(df2$phi_type,levels=c( "~norm(0,1)","~norm(1,1)","~unif(0,1)","~norm(0,1)ber(0.2)"),labels=c(expression(paste(Phi[i],'~Normal(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(1,1)')),
                                                                                             expression(paste(Phi[i],'~Uniform(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(0,1) x Bernoulli(0.2)'))))
df3$phi_type= factor(df3$phi_type,levels=c( "~norm(0,1)","~norm(1,1)","~unif(0,1)","~norm(0,1)ber(0.2)"),labels=c(expression(paste(Phi[i],'~Normal(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(1,1)')),
                                                                                             expression(paste(Phi[i],'~Uniform(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(0,1) x Bernoulli(0.2)'))))
df4$phi_type= factor(df4$phi_type,levels=c( "~norm(0,1)","~norm(1,1)","~unif(0,1)","~norm(0,1)ber(0.2)"),labels=c(expression(paste(Phi[i],'~Normal(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(1,1)')),
                                                                                             expression(paste(Phi[i],'~Uniform(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(0,1) x Bernoulli(0.2)'))))
df5$phi_type= factor(df5$phi_type,levels=c( "~norm(0,1)","~norm(1,1)","~unif(0,1)","~norm(0,1)ber(0.2)"),labels=c(expression(paste(Phi[i],'~Normal(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(1,1)')),
                                                                                             expression(paste(Phi[i],'~Uniform(0,1)')),
                                                                                             expression(paste(Phi[i],'~Normal(0,1) x Bernoulli(0.2)'))))
pB <- ggplot(df1,
  ...
  facet_wrap(phi_type~.,scales='free_y',ncol=1,nrow=4, labeller = label_parsed) +scale_y_continuous(breaks=equal_breaks2(n=3, s=0.05))
...
pE <- ggboxplot(df4,x='Treatment',y='Q',col='Treatment',palette = "dark2",
  ...
  facet_wrap(~phi_type,scales='free_y',ncol=1,nrow=4, labeller = label_parsed)  +
...
left = ggarrange(pB,pD,nrow=1,ncol=2,labels=c('A','B'),common.legend = TRUE)
right = ggarrange(pE,pF,pG,ncol=3,nrow = 1,labels=c('C','D','E'),align = 'h')
ggsave('../Plots/S16.png',ggarrange(left,right,nrow=2,ncol=1),height=12,width=9)