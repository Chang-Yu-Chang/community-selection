...
mapping_file = mapping_file[l ==0 & response == "type III" & 
...
mapping_file2 = mapping_file2[l ==0 &  response == "type III" & 
                                selected_function == 'f1_additive' & 
                                cost_distribution=='Norm' & phi_distribution=='Norm' & phi_mean ==0 ,]
...
mapping_file3 = mapping_file3[l ==0 &  response == "type III" & 
                                selected_function == 'f1_additive' & 
                                cost_distribution=='Norm' & phi_distribution=='Norm' & phi_mean ==0 ,]
...
# df1 = df1[df1$metacommunity_sampling != 'Non-Additive Rugged',]
for(j in 1:nrow(df1)){
  k = df1$seed[j]
  screen = fread(mapping_file[seed==k & 
                                protocol=='simple_screening' &
                                monoculture==FALSE & 
                                metacommunity_sampling ==df1$metacommunity_sampling[j]]$file)
...
for(j in 1:nrow(df2)){
  ...
  screen = fread(mapping_file[seed==k & 
                                protocol=='simple_screening' &
                                monoculture==FALSE & 
                                metacommunity_sampling ==df2$metacommunity_sampling[j]]$file)
...
# df2 = df2[!(DE_Strategy == 'Resource Shift' & metacommunity_sampling == 'Target Resource')]  #Rshift strategy not applicable to this function
...
df3[,rank :=frank(-CommunityPhenotype),by=list(seed,Transfer,bottleneck_size,metacommunity_sampling)]
df3 = df3[bottleneck_size %!in% c(1e-4,1e-5)]
df3 = df3[Transfer==40]
df3$Popsize_mean = 1000*df3$scale*df3$bottleneck_size*df3$dilution
df3[,F_Mean := mean(CommunityPhenotype),by = list(seed,Transfer,bottleneck_size,metacommunity_sampling)]
df3[,F_Max := max(CommunityPhenotype),by = list(seed,Transfer,bottleneck_size,metacommunity_sampling)]
df3[,Mean_F_Mean := mean(F_Mean),by = list(Popsize_mean,metacommunity_sampling)]
df3[,SD_F_Mean := sd(F_Mean),by = list(Popsize_mean,metacommunity_sampling)]
df3[,Mean_F_Max := mean(F_Max),by = list(Popsize_mean,metacommunity_sampling)]
df3[,SD_F_Max := sd(F_Max),by = list(Popsize_mean,metacommunity_sampling)]
df3 = df3[Well == 'W0']
...
df4_control[,Control_Maximum:=max(CommunityPhenotype),by=list(exp_id,Transfer,metacommunity_sampling)]
df4_control = df4_control[Well == 'W0' & Transfer ==40]
df4_control = df4_control[order(df4_control$exp_id),]
df4_control = df4_control[,c('seed','metacommunity_sampling','Control_Maximum')]
...
df4 = merge(df4,df4_control,by=c('seed','metacommunity_sampling'))
...
df5[,Fmax :=max(CommunityPhenotype),by=list(exp_id,Transfer,metacommunity_sampling)]
df5[,R :=mean(RS),by=list(exp_id,Transfer,metacommunity_sampling)]
df5[,Fmean :=mean(CommunityPhenotype),by=list(exp_id,Transfer,metacommunity_sampling)]
df5$Fmax2 = t20_2$Fmax
df5 = df5[Well=='W0']
my_comparisons2 <- list( c("NS", "DE"), c("DE", "Synthetic"), c("NS", "Synthetic") )

df1[df1$metacommunity_sampling =='Default']$metacommunity_sampling = 'Fixed'
df2[df2$metacommunity_sampling =='Default']$metacommunity_sampling = 'Fixed'
df3[df3$metacommunity_sampling =='Default']$metacommunity_sampling = 'Fixed'
df4[df4$metacommunity_sampling  =='Default']$metacommunity_sampling = 'Fixed'
df5[df5$metacommunity_sampling  =='Default']$metacommunity_sampling =  'Fixed'
...
pB <- ggplot(df1,
  ...
  facet_wrap(metacommunity_sampling~.,scales='free_y',ncol=1,nrow=3) +scale_y_continuous(breaks=equal_breaks2(n=3, s=0.05))
...
pE <- ggboxplot(df4,x='Treatment',y='Q',col='Treatment',palette = "dark2",
  ...
  facet_wrap(~metacommunity_sampling,scales='free_y',ncol=1,nrow=3)  +
...
img1 =  readPNG( "../Plots/Cartoons/FigSX_sampling.png", TRUE)
...
ggsave('../Plots/S15.png',ggarrange(left,right,nrow=2,ncol=1),height=12,width=9)